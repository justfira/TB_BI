...
return new class extends Migration
{
    ...
    {
        // Fast ETL: parse JSON once into temp table, then set-based INSERT IGNORE.
        $sql = <<<'SQL'
...
BEGIN
    ...
    -- === Step 0: Materialize JSON once (single scan of staging_workorder) ===
    ...
    SELECT
        ...
        NULLIF(TRIM(COALESCE(
            JSON_UNQUOTE(JSON_EXTRACT(sw.data_json, '$.sto')),
            'UNKNOWN'
        )), ''),
        NULLIF(TRIM(COALESCE(
        ...
        NULLIF(TRIM(COALESCE(
            JSON_UNQUOTE(JSON_EXTRACT(sw.data_json, '$.kendala_pt1')),
            'UNKNOWN'
        )), ''),
        ...
        NULLIF(TRIM(COALESCE(
            JSON_UNQUOTE(JSON_EXTRACT(sw.data_json, '$.nik_teknisi')),
            'UNKNOWN'
        )), ''),
    ...
    -- Keep first row per wo_sc_id for fact insert
    ...
    ALTER TABLE tmp_etl_deduped
        ...
        ADD KEY idx_wo_sc_id (wo_sc_id),
        ADD KEY idx_tanggal (tanggal),
        ADD KEY idx_sto (sto(100)),
        ADD KEY idx_status_wo (status_wo(100)),
        ADD KEY idx_kendala_pt1 (kendala_pt1(191)),
        ADD KEY idx_nik_teknisi (nik_teknisi),
        ADD KEY idx_track_id (track_id(100));

    -- === Step 1-6: Dimension upserts via INSERT IGNORE (unique keys) ===
    ...
    SELECT
        t.kendala_pt1, t.kategori_roc, t.kategori_solusi, t.solusi_kendala, NOW(), NOW()
    ...
    SELECT DISTINCT
        t.track_id, t.nama_pelanggan, t.nama_contact, t.segment, t.layanan, t.alamat_instalasi, t.uic, t.koordinat_pelanggan,
        NOW(), NOW()
    FROM tmp_etl_rows t
    WHERE t.track_id IS NOT NULL AND t.track_id <> '';

    -- === Step 7: dim_infrastruktur (anti-join, indexed on wo_id) ===
    ...
    WHERE t.wo_sc_id IS NOT NULL AND t.wo_sc_id <> '' AND di.id IS NULL;

    -- === Step 8: fact_workorder ===
    ...
    FROM fact_workorder
    WHERE etl_log_id = p_log_id;

    SELECT COUNT(*) INTO v_duplicate
    FROM tmp_etl_rows t
    WHERE t.wo_sc_id IS NOT NULL AND t.wo_sc_id <> ''
      AND (
          t.staging_id NOT IN (SELECT staging_id FROM tmp_etl_deduped)
          OR EXISTS (SELECT 1 FROM fact_workorder fw WHERE fw.wo_sc_id = t.wo_sc_id AND fw.etl_log_id <> p_log_id)
      );

    SET v_failed = GREATEST(v_total - v_success - v_duplicate, 0);

    -- === Step 9: fact_kendalateknis ===
...
SQL;
        ...
        DB::unprepared('DROP PROCEDURE IF EXISTS sp_etl_workorder_optimized');
