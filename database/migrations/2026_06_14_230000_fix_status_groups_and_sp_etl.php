...
return new class extends Migration
{
    ...
    {
        // 1. Update existing dim_status groups to follow Laravel DimStatus constants:
        //    'Selesai', 'Pending', 'Gagal'
        DB::statement("
            UPDATE dim_status 
            SET status_group = CASE 
                WHEN LOWER(status_wo) REGEXP 'selesai|close|done|completed' THEN 'Selesai'
                WHEN LOWER(status_wo) REGEXP 'gagal|cancel|reject|workfail|nok' THEN 'Gagal'
                ELSE 'Pending'
            END
        ");

        // 2. Update existing fact_workorder records' flags
        DB::statement("
            UPDATE fact_workorder fw
            INNER JOIN dim_status ds ON fw.status_id = ds.status_id
            SET fw.is_sla_tercapai = CASE WHEN ds.status_group = 'Selesai' THEN 1 ELSE 0 END,
                fw.is_workfail = CASE WHEN ds.status_group = 'Gagal' THEN 1 ELSE 0 END
        ");

        // 3. Drop and Recreate the stored procedure sp_etl_workorder with correct status group logic
...
BEGIN
    ...
    -- Insert Status (Correct logic using REGEXP and Indonesian format)
    ...
    SELECT
        ...
        CASE WHEN dstatus.status_group = 'Gagal' THEN 1 ELSE 0 END,
...
SQL);
    ...
    {
        // Revert groups of dim_status to SELESAI / INPROGRESS just in case
        DB::statement("
            UPDATE dim_status 
            SET status_group = CASE 
                WHEN status_group = 'Selesai' THEN 'SELESAI'
                ELSE 'INPROGRESS'
            END
        ");
